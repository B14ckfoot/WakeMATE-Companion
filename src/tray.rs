...
const POPUP_LOGICAL_WIDTH: f64 = 252.0;
...
impl TrayApp {
    fn new(
        ...
        Ok(Self {
            config: config.clone(),
            assets_dir,
            server: ServerThread::spawn(config)?,
            tray_icon: None,
            pairing_popup: None,
            menu_ids: None,
            startup_error: None,
    ...
    fn handle_menu_event(
        ...
        if event.id == menu_ids.show_pairing_qr {
            self.toggle_pairing_qr_popup(event_loop)?;
        } else if event.id == menu_ids.rotate_pairing_token {
...
fn render_pairing_popup(
    ...
    let device_label = popup_device_label(device_name, 24);
...
fn popup_layout(width: u32, height: u32, qr_modules: i32) -> PopupLayout {
    ...
    let header_gap = (6 * title_scale as i32).max(10);
    let title_y = 18;
    let subtitle_y = title_y + title_height + header_gap;
    let device_y = subtitle_y + subtitle_height + 12;

    let qr_total_modules = (qr_modules + 8).max(1) as u32;
    let max_qr_size = width
        .saturating_sub(52)
        .min(height.saturating_sub((device_y + 72).max(0) as u32));
    ...
    let qr_y = device_y + 18;
    let footer_y = qr_y + qr_size as i32 + 20;
