...
- Windows does not run a pre-login Companion API. Direct phone Wake-on-LAN works without it; pairing, status, and control start with the normal-user tray. Stale `--headless-server` tasks must exit without reading user state or binding listeners.
...
- Windows installer: `powershell -NoProfile -ExecutionPolicy Bypass -File .\installer\build-installer.ps1` (always performs a fresh release build first).
...
- Keep public, authenticated, pairing, input, and power capabilities explicitly separated. A request reaching the server never implies permission to execute it.
...
- Keep Windows-only code behind compile-time guards. Runtime operation must use normal-user privileges; admin work is limited to machine-scoped installation, prerequisites, firewall configuration, and cleanup of legacy installer state.
